...
document.querySelectorAll('[data-download]').forEach(button=>button.onclick=()=>{window.location.href=`http://127.0.0.1:8091/admin/template/${button.dataset.download}`});
document.querySelectorAll('[data-preview]').forEach(button=>button.onclick=()=>{const dialog=$('preview');$('preview-frame').src=`http://127.0.0.1:8091/admin/preview/${button.dataset.preview}`;dialog.showModal()});
