...
async function adminDocument(kind,preview=false){const response=await fetch(`http://127.0.0.1:8091/admin/${preview?'preview':'document'}/${kind}`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(last)});if(!response.ok)throw Error('문서 생성에 실패했습니다.');return response.blob()}
