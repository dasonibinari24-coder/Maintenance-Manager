...
function startWizard(kind, grade, area) {
  addOfficialFields(); wizard.classList.remove('hidden');
  ...
  form.elements.buildingArea.value = area || ''; form.elements.managerGrade.value = grade;
  if (!form.elements.reportDate.value) form.elements.reportDate.value = new Date().toISOString().slice(0, 10);
  current = 1; renderWizard(); wizard.scrollIntoView({behavior: 'smooth', block: 'start'}); focusActiveField();
}
function renderWizard() {
  ...
  prev.hidden = current === 1; next.hidden = current === 4; draft.hidden = current !== 4;
  const certificate = certificateButton(); if (certificate) certificate.hidden = current !== 4;
  const preview = document.querySelector('#open-original-preview'); if (preview) preview.hidden = current !== 4;
  doc24.hidden = true; if (current === 4) review();
}
function validStep() {
  ...
  if (invalid.length) { invalid[0].focus(); return false; } return true;
}
function review() {
  const data = new FormData(form);
  const names = {buildingName: '건축물 명칭', buildingUse: '건축물 용도', buildingAddress: '건축물 주소', ownerName: '관리주체명', ownerRepresentative: '대표자', managerName: '유지관리자', managerGrade: '기술자 등급', appointmentDate: '선임일', reportDate: '신고일', certificateReason: '신청사유'};
  document.querySelector('#review-list').innerHTML = Object.entries(names).map(([key, name]) => `<div><span>✓</span><b>${name}</b><em>${data.get(key) || '-'}</em></div>`).join('');
}
next.addEventListener('click', () => { if (validStep()) { current++; renderWizard(); focusActiveField(); } });
prev.addEventListener('click', () => { current--; renderWizard(); focusActiveField(); });
